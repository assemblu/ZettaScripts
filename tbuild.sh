...
cmake -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX=/home/emir/dev/cyclonedds-certified/install -DCMAKE_PREFIX_PATH=/home/emir/dev/iceoryx/build -DSANITIZER=address -DENABLE_SSL=off -DENABLE_SECURITY=off -DENABLE_LIFESPAN=off -DENABLE_DEADLINE_MISSED=on -DENABLE_TYPE_DISCOVERY=off -DENABLE_TOPIC_DISCOVERY=off -DENABLE_SHM=off -DBUILD_TESTING=on -DBUILD_IDLC_XTESTS=off -DBUILD_EXAMPLES=on -DZPMALLOC=off -DWERROR=on -DCMAKE_EXPORT_COMPILE_COMMANDS=1 -DENABLE_ICEORYX=on . -Bbuild

# cmake -DCMAKE_BUILD_TYPE=Debug -DCMAKE_INSTALL_PREFIX=/home/emir/dev/cyclonedds-certified/install -DSANITIZER=none -DENABLE_SSL=off -DENABLE_SECURITY=on -DENABLE_LIFESPAN=off -DENABLE_DEADLINE_MISSED=on -DENABLE_TOPIC_DISCOVERY=off -DENABLE_TYPE_DISCOVERY=off -DENABLE_SHM=on -DBUILD_TESTING=on -DBUILD_IDLC_XTESTS=on -DBUILD_EXAMPLES=on -DZPMALLOC=off -DWERROR=on -DCMAKE_EXPORT_COMPILE_COMMANDS=1 . -Bbuild
